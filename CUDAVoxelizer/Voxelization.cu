...
__global__ void voxelizeComponent(const size_t numFaces, const size_t offsetVertices, const size_t offsetIndices, const size_t numSamples, int* grid, const AlgGeom::VAO::Vertex* vertices, const unsigned* faceIndices, const vec2* noise)
{
	...
	{ 
		...
		vec3 v1 = vertices[faceIndices[faceIdx + offsetIndices + 0] + offsetVertices]._position, 
			 v2 = vertices[faceIndices[faceIdx + offsetIndices + 1] + offsetVertices]._position, 
			 v3 = vertices[faceIndices[faceIdx + offsetIndices + 2] + offsetVertices]._position;
